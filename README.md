...
- **Large-scale validation**:
  ...
  - Test-time: Two datasets with 8+12 models, 10 NLP datasets, up to 10,000 samples per question
...
│   ├── monkey_query/                    # Query LLMs → ResMAT1 (10k samples, 8 models)
│   ├── monkey_query_2/                  # Query LLMs → ResMAT2 (2.5k samples, 12 models)
│   ├── monkey_gather_data/              # Collect data → ResMAT1 (HELM validation)
│   ├── monkey_gather_data_2/            # Collect data → ResMAT2 (recent models)
│   └── monkey_analysis/                 # Analysis and plotting (uses both ResMAT1 & ResMAT2)
...
- `monkey_*_2/`: Not version 2, but generates dataset 2 (ResMAT2) as described in paper Section 4.2
...
The paper uses **TWO** test-time datasets (as described in Section 4.2):

#### Dataset 1: $P^{\text{testtime}}_1$ (ResMAT1)
- **Purpose**: HELM validation, z-score correlation analysis
- **Models**: 8 LLMs (Llama-3, Qwen3, DeepSeek, Gemma)
- **Questions**: 552 questions from 6 benchmarks (commonsense, legalbench, med_qa, bbq, lsat_qa, legal_support)
- **Samples**: 10,000 per question
- **Prompting**: Few-shot, temperature 1.0
- **Data source**: `stair-lab/irsl_testtime_resmat1`
- **Generated by**: [`monkey/monkey_query/`](monkey/monkey_query/) + [`monkey/monkey_gather_data/`](monkey/monkey_gather_data/)

#### Dataset 2: $P^{\text{testtime}}_2$ (ResMAT2)
- **Purpose**: Recent model experiments, main paper figures
- **Models**: 12 latest LLMs (DeepSeek-R1, Qwen3, QwQ, Gemma-3)
- **Questions**: 120 questions from 4 latest benchmarks (AIME 2024, AIME 2025, MMLU Pro, Global MMLU Lite)
- **Samples**: ~2,500 per question (actual: 2,560 in most cases)
- **Prompting**: Zero-shot chain-of-thought, temperature 0.6
- **Data source**: `stair-lab/irsl_testtime_resmat2`
- **Generated by**: [`monkey/monkey_query_2/`](monkey/monkey_query_2/) + [`monkey/monkey_gather_data_2/`](monkey/monkey_gather_data_2/)
- **Quality control**: 12 models excluded, 19 MMLU Pro questions excluded (see [README](monkey/monkey_gather_data_2/README.md))
