...
if __name__ == "__main__":
    ...
    for model in tqdm(MODELS):
        # with open(f"{cache_dir}/resmat_aggregated/results_{model}.pkl", "rb") as f:
        # with open(f"/lfs/skampere2/0/sttruong/irsl/data/pretrain_helm/results_Amber_sttruong_skampere2.pkl", "rb") as f:
        with open(f"/lfs/skampere2/0/sttruong/irsl/data/pretrain_helm/results_pythia-12b_sttruong_skampere2.pkl", "rb") as f:
        ...
        for scenario in tqdm(SCENARIOS):
            ...
            ys_np = ys.cpu().numpy().flatten()  # flatten to 1D for distribution
            plt.figure(figsize=(6,4))
            plt.hist(ys_np, bins=50, color="steelblue", alpha=0.7, edgecolor="black")
            plt.xlabel("Value")
            plt.ylabel("Frequency")
            plt.title("Distribution of ys values")
            plt.tight_layout()
            plt.savefig(f"{output_dir}/ys_distribution.png", dpi=100)
            plt.close()
    ...
    #         def _run_one(i):
    #             return cat_beta(ys[i], zs, device, budget, init_frac)
    ...
    #             budgets = np.arange(budget - int(budget * init_frac) + 1)
