...
if __name__ == "__main__":
    ...
    data = torch.load(f"{input_dir}/math_tensor.pth", map_location="cpu")
    ...
    probs /= probs.sum()
    test_idxs = torch.multinomial(probs, split_idx, replacement=False).tolist()
    train_idxs = [i for i in range(n_items) if i not in test_idxs]
