...
from monkey_query_utils import create_prompts_and_answers_zero_shot, exact_match, quasi_exact_match, is_equiv_chain_of_thought, advanced_gsm_eval
...
def generate_outputs_from_model(
    ...
    policy_model_sampled_outputs_dir = os.path.join(
        "../../data/monkey_query/eval_results_zero_shot",
        dataset,
        model_nickname.split("/")[-1],
    )
    os.makedirs(policy_model_sampled_outputs_dir, exist_ok=True)

    data: Dict[str, List[str]] = create_prompts_and_answers_zero_shot(
        dataset=dataset,
...
def sample_outputs_from_policy_model_and_write_to_disk(
    ...
    for problem, prompt_idx, prompt, solution in zip(
        ...
        for batch_generation_idx in range(
            ...
            scores = [float(evaluate_fn(solution, response)) for response in responses]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--num_prompts_to_use", type=int, default=1319, help="Number of prompts to use"
