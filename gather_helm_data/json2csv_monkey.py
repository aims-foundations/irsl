...
if __name__ == "__main__":
    ...
    results = []
    for d_requests, d_predictions, run_specs, scenario_states, paths in tqdm(zip(*all_lists, all_paths), total=len(all_lists[0])):
        ...
        scenario_states = pd.json_normalize(scenario_states["request_states"])
        ...
        result = pd.concat([d_requests, d_predictions, run_specs, scenario_states], axis=1)
    ...
    results.to_pickle(f"{output_dir}/responses_monkey_{benchmark_name}.pkl")
